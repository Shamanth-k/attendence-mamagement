...
function AttendancePage() {
  ...
  const onPeriodChange = (option) => {
    setPeriod(option);
    if (option === "date-range" && customFrom === customTo) {
      setCustomFrom(monthStart(anchorDate));
      setCustomTo(monthEnd(anchorDate));
    }
  };
    ...
    <div className="screen-card attendance-page">
      <div className="attendance-hero">
        <div>
          ...
          <h2 className="attendance-title">{selectedEmployee?.full_name || "Select Team Member"}</h2>
        </div>
        ...
        <div className="table-scroll">
          <table className="data-table attendance-table">
            ...
            <tbody>
              {rows.length === 0 ? (
                <tr>
                  <td colSpan={17} className="attendance-empty-row">
